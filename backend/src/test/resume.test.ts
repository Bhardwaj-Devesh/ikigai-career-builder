...
async function testResumeUpload() {
  try {
    ...
    // 3. Upload resume
    const response = await fetch('https://ikigai-career-backend.onrender.com/api/resume/upload', {
      method: 'POST',
      headers: {
    ...
    const getResponse = await fetch(`https://ikigai-career-backend.onrender.com/api/resume/user-resume/${testUser.id}`);
